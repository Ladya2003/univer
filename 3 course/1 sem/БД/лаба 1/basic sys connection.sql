...
INSERT INTO Categories (CategoryName, ParentID, NodeLevel, NodePath)
VALUES ('SUVs', NULL, 0, '/');
INSERT INTO Categories (CategoryName, ParentID, NodeLevel, NodePath)
VALUES ('Convertibles', NULL, 0, '/');
COMMIT;
select * from Categories;

CREATE OR REPLACE PROCEDURE DisplayNodes(NodePath NVARCHAR2) AS
BEGIN
    FOR c IN (SELECT CategoryID, CategoryName, NodePath, NodeLevel
              FROM Categories
              WHERE CONNECT_BY_ISLEAF = 1
              START WITH NodePath = NodePath
              CONNECT BY PRIOR CategoryID = ParentID)
    LOOP
        DBMS_OUTPUT.PUT_LINE('CategoryID: ' || c.CategoryID || ', CategoryName: ' || c.CategoryName || ', NodePath: ' || c.NodePath || ', NodeLevel: ' || c.NodeLevel);
    END LOOP;
END DisplayNodes;
/
...
ADD HierarchyPath VARCHAR2(1000) DEFAULT '/';

CREATE OR REPLACE PROCEDURE DisplayChildNodes(p_HierarchyPath IN VARCHAR2) IS
BEGIN
    FOR r IN (
        SELECT 
            c.CategoryID, 
        ...
        WHERE c.HierarchyPath LIKE p_HierarchyPath || '%'
        ORDER BY c.HierarchyPath
    ) LOOP
        DBMS_OUTPUT.PUT_LINE('ID: ' || r.CategoryID || ', Name: ' || r.CategoryName || ', HierarchyPath: ' || r.HierarchyPath || ', Parent: ' || NVL(r.ParentCategoryName, 'ROOT'));
    END LOOP;
END DisplayChildNodes;
/




CREATE OR REPLACE PROCEDURE AddChildNode(p_CategoryName IN VARCHAR2, p_ParentPath IN VARCHAR2) IS
    v_NewPath VARCHAR2(1000);
BEGIN
    -- Определить новый путь для узла
    SELECT p_ParentPath || CategoryID || '/' INTO v_NewPath
    FROM Categories
    WHERE HierarchyPath = p_ParentPath;

    INSERT INTO Categories (CategoryName, HierarchyPath)
    VALUES (p_CategoryName, v_NewPath);
...
CREATE OR REPLACE PROCEDURE MoveSubTree(p_FromPath IN VARCHAR2, p_ToPath IN VARCHAR2) IS
BEGIN
    UPDATE Categories
    SET HierarchyPath = REPLACE(HierarchyPath, p_FromPath, p_ToPath || '/')
    WHERE HierarchyPath LIKE p_FromPath || '%';
...
EXEC AddChildNode('НоваяКатегория4', '/1/');
EXEC DisplayChildNodes('/');
...
ALTER TABLE Categories 
ADD ParentCategoryID NUMBER REFERENCES Categories(CategoryID);
select * from Categories;

CREATE OR REPLACE PROCEDURE DisplayChildCategories(p_CategoryID IN NUMBER) IS
BEGIN
    FOR r IN (
        SELECT CategoryID, CategoryName, LEVEL as HierarchyLevel
        FROM Categories
        START WITH CategoryID = p_CategoryID
        CONNECT BY NOCYCLE PRIOR CategoryID = ParentCategoryID
    ) LOOP
        DBMS_OUTPUT.PUT_LINE('ID: ' || r.CategoryID || ', Name: ' || r.CategoryName || ', Level: ' || r.HierarchyLevel);
    END LOOP;
END DisplayChildCategories;
/

EXEC DisplayChildCategories(2);


-- 3.3
CREATE OR REPLACE PROCEDURE AddChildCategory(p_CategoryName IN VARCHAR2, p_ParentCategoryID IN NUMBER) AS
BEGIN
...
EXEC AddChildCategory('Новая категория3', 3);



-- 3.4
CREATE OR REPLACE PROCEDURE MoveSubtree(p_MovingNodeID IN NUMBER, p_TargetNodeID IN NUMBER) IS
    v_PotentialCycleNode NUMBER;
BEGIN
    -- Перемещение ветки
    UPDATE Categories 
    SET ParentCategoryID = p_TargetNodeID 
    WHERE CategoryID IN (
        SELECT CategoryID
        FROM Categories
        START WITH CategoryID = p_MovingNodeID
        CONNECT BY NOCYCLE PRIOR CategoryID = ParentCategoryID
    );
    COMMIT;
...
BEGIN
    MoveSubtree(1, 2); -- Например, перемещение узла с ID 1 под узел с ID 2
END;
/
...
-- Пожалуйста, укажите нужные значения для ClientID, OrderDate, OrderAmount и EmployeeID
