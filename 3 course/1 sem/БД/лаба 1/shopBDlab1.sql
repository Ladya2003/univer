...
UPDATE OrderedProducts_Purchase SET Quantity = 6 WHERE PurchaseOrderID = 1 AND ProductID = 1;
...
DELETE FROM OrderedProducts_Purchase WHERE PurchaseOrderID = 3 AND ProductID = 3;
...
-- Создайте необходимые индексы для других таблиц, если нужно
...
-- Пример вызова триггера (для демонстрации, не обязательно выполнять в реальном приложении)
...
-- 10.1
CREATE PROCEDURE dbo.ReadTextFileProcedure
    @filePath NVARCHAR(MAX),
    @fileContent NVARCHAR(MAX) OUTPUT
AS
EXTERNAL NAME Database1.TextFileReader.ReadTextFile;

DECLARE @fileContent NVARCHAR(MAX);

EXEC dbo.ReadTextFile
    @filePath = 'D:\hello.txt',
    @fileContent = @fileContent OUTPUT;

SELECT @fileContent AS FileContent;
