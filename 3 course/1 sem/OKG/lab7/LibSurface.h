...

{
	...
	for (double fi = 0; fi < 360.0; fi += df)		  // Азимут
		for (double q = 0; q <= 180.0; q += dq)		  // между вект наблюдения и плоскостью XY
		{
			VSphere(1) = fi;
			VSphere(2) = q;
			CMatrix VCart = SphereToCart(VSphere);	  // т.сферы в декарт                                     
			VSphereNorm = VCart;				      // В-р нормали
			double cos_RN = CosV1V2(VR, VSphereNorm); // наблюдения и нормаль точки
			...
			if (cos_RN > 0)
			{
				...
				{		
					if (Index == 0)				///диффуз
						kLight = cos_PN * cos_RN; // падение луча и угл между наблюдением
					else if (Index == 1)		///зеркал
						kLight = pow(CosV1V2(VSphereNorm, VS), 12);// косинус между вектором нормали и источником наблюдения
