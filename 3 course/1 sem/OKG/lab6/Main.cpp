...
void CMainWnd::OnCPlot3DDef()
{
	...
	CRect  WinRect;
	GetClientRect(WinRect);			//получаем рабочую область
	WinRect.SetRect(WinRect.left + 50, WinRect.top + 50, WinRect.right - 50, WinRect.bottom - 50);	//задаем рабочую область
	Graph1.SetFunction(Function1, SpaceRect, dx, dy);	//задаем функцию
	Graph1.SetViewPoint(r, fi, q);	//делаем точку просмотра
	Graph1.SetWinRect(WinRect);		//задаем рабочую область окна
	Index = 1;
	Invalidate();
}
...
void CMainWnd::OnCplot3dFunc1()
{
	...
	GetClientRect(WinRect);              //получаем рабочую область
	WinRect.SetRect(WinRect.left + 50, WinRect.top + 50, WinRect.right - 50, WinRect.bottom - 50); //задаем рабочую область
	Graph2.SetFunction(Function2, SpaceRect, dx, dy); //задаем функцию
	Graph2.SetViewPoint(r, fi, q);       //делаем точку просмотра
	Graph2.SetWinRect(WinRect);          //получаем рабочую область окна
	Index = 2;
	Invalidate();
}
...
void CMainWnd::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)//камеры
{
	CMatrix P(3);
	switch (Index)
	{//
...
CMainWnd::CMainWnd()
{
	Create(NULL, L"lab6", (WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_MAXIMIZEBOX) &~(WS_THICKFRAME), rectDefault, NULL, NULL);
}
