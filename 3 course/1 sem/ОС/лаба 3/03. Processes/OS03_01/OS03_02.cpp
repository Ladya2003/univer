...
// содержит информацию о том, как операционная система должна создать новый процесс, включая параметры командной строки, окружение, дескрипторы файлов и другие настройки.
// PROCESS_INFORMATION pi, pi1; // информация о созданном процессе и его первичном потоке
// Она включает идентификатор процесса (PID) и дескрипторы для управления процессом.
...
// 1 параметр указывает на имя исполняемого модуля, который следует запустить в новом процессе.
