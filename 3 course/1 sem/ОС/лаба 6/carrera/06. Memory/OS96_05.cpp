...
int main()
{
    // Создаем пользовательскую кучу с размером 4MB
    ...
    int* mm = (int*)HeapAlloc(userHeap, HEAP_NO_SERIALIZE | HEAP_ZERO_MEMORY, 300000 * sizeof(int)); // Выделяем память с помощью пользовательской кучи
