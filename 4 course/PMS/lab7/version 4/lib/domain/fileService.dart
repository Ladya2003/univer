...
class FileService {
  ...
  Future<Directory> _getDirectory(String dirType) async {
    if (Platform.isAndroid && dirType == 'Application Library') {
      throw Exception('Нет доступа к Application Library на Android');
    }
    ...
    switch (dirType) {
      ...
      case 'External Storage':
        ...
        if (externalStorageDir == null) {
          throw Exception('Директория внешнего хранилища недоступна.');
        }
        return externalStorageDir;
      case 'Downloads':
        final dirPath = (await getDownloadsDirectory())?.path ?? '';
        print(dirPath);
        if (dirPath.isEmpty) {
          throw Exception('Директория загрузок недоступна.');
        }
